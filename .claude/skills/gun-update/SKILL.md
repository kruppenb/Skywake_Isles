---
name: gun-update
description: Upgrade the next procedural gun (scatter, repeater, burst or longshot) to a Meshy-built GLB the way the flintlock was done — the lead plans, reviews plates/renders/shots and judges in-engine; Sonnet/Opus subagents generate, fit, wire, test and tune; credits are spent only after the lead approves the plates. Use when the user says "upgrade the next gun", "/gun-update <kind>", or asks for a GLB model of one of the guns.
argument-hint: <kind>   (scatter | repeater | burst | longshot)
---
...
`docs/WEAPON_MODELS.md` (pipeline, gun-space contract, the flintlock's exact commands),
`client/weapon-models.js` (the runtime swap layer), `client/models.js` lines 617–720
(`WEAPON_HANDLING`, `buildWeapon`), `test/weapon-assets.test.js` (binary contract) and
`test/weapon-models.test.js`. The flintlock is the worked example; every number below was measured.
...
Contract ranges are a guard against a mis-fit, not a design. Starting points per kind for the
test's per-kind table (confirm after the pass-1 landmark print, widen only with a reason, and never
loosen the muzzle rule): scatter zMin [−1.08,−.88], zMax .60, yMin [−.60,−.20], yMax .55, ǀxǀ .30,
barrel band z [−.60,−.35] (skip the flare), action y [.05,.55] rotating; repeater zMin [−1.10,−.90],
...
`build_weapon.py --kind <kind> --length … --muzzle-z … --bore-y … --allow-misfit` →
`render_glb.py --wide` → the agent scans the pass-1 body in gun space for the action part (the
script's `hammerHint` is only a hint) → pass 2 with `--action-box`, `--hinge`, `--hinge-axis`,
...
without an action and record it.

## Phase 3 — runtime and test deltas (Opus, can run in parallel with Phase 2)

- `test/weapon-assets.test.js`: turn the single `CONTRACT` into a per-kind table keyed by manifest
  kind; the flintlock keeps its numbers verbatim; add the new kind's ranges from the spec. The
  action rule must allow translating parts (negative y for magazines).
- `client/weapon-models.js`: add the kind to `WEAPON_ASSET_URLS` and a `WEAPON_ASSET_HANDLING`
  ...
  needed; if one is, the agent reports why instead of improvising in the frame code.
- `test/weapon-models.test.js`: extend the handling cases for the new kind; a translating action
  needs a case proving the hinge chain carries the frame code's translation through unchanged.
...
`node tools/qa/weapons/game-check.mjs` (join as Watcher, walk to a launch gate and press E, glide,
land, aim, fire; the lobby is consumed per run, restart the server between runs). The agent tries
...
re-runs the tests and the full capture. **Lead looks at the side close-up, the reload close-up and
the in-game shot.** Expect zero console errors and warnings; the missing-asset path is already
covered (one warning, procedural gun).
