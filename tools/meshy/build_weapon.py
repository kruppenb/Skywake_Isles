...
CONTRACTS = {
    ...
    # still at y .17 but running to a muzzle at z -1.46 -- so the z numbers move and little else
    # does. A stock-wrist band with zFrom -.02, so neither the curved magazine nor the trigger-guard
    # bow counts as grip and the centroid still has to land on the pistol grip / stock wrist at
    # z .05 .. .40 -- but the band itself is y -.10..+.02, not the repeater's -.20..-.08: Meshy
    # built this gun's whole lower half shallower than the plate (the bore sits 34 % down the
    # silhouette where the plate has it 23 % down), so the pistol-grip belly bottoms out at y -.030
    # (z .05 .. .08), the stock wrist rises to +.02 and the butt toe is y -.035 (z .43 .. .45); the
    # trigger guard bottoms at +.033, just above the band. Its action is the magazine hanging *under* the receiver,
    # which the frame code TRANSLATES rather than rotates (--hinge-axis 0 0 1, an identity hinge
    # frame; the burst adds +.04 z to the repeater's stroke), so the action y band is negative;
    # reach .40 covers the magazine's far bottom corner from a pivot at its top centre. Measured
    # off the approved side plate at length 1.91 with the bore at y .17 (procedural numbers in
    # brackets): Gemini drew the fore-end running forward to z -1.01 (-.77), so the barrel band is
    # z -1.30..-1.05, ahead of the wood and behind the brass muzzle ring at z -1.46..-1.38, and
    # samples bare tube rather than wood, bands or ring; the magazine sits at z -.46..-.30
    # (-.29..-.03), ahead of the trigger guard at z -.27..-.10, and is the gun's lowest point with
    # its brass base plate near y -.31 (-.47), so yMin's upper bound is -.18, not the grip's; its
    # neck meets the receiver underside near y +.05 (-.09), inside the action band's .10 ceiling
    # (the built mesh: base plate y -.24, neck cut at y .055, magazine z -.51..-.30). Muzzle y is
    # the bore +- .05, never loosened.
    'burst': {
        'bounds': {'zMin': (-1.56, -1.36), 'zMax': .60, 'yMin': (-.65, -.18), 'yMax': .55, 'absX': .25},
        'muzzle': {'zBelowMin': -.02, 'zAboveMin': .06, 'y': (.12, .22), 'absX': .03},
        'grip': {'band': (-.10, .02), 'zFrom': -.02, 'absCentroidX': .06, 'centroidZ': (.05, .40),
                 'extentX': .30, 'extentZ': .50},
        'barrel': {'band': (-1.30, -1.05), 'centroidY': (.02, .25), 'absCentroidX': .03,
                   'extentX': .25, 'extentY': .40},
        'action': {'reach': .40, 'y': (-.60, .10)},
    },
    # The longshot: the longest gun and the only tall one -- 2.35 muzzle-to-butt with the bore still
    # at y .17, running to a muzzle at z -1.90 -- so every z number stretches again, and the
    # telescopic sight standing above the receiver is what moves y. Measured off the approved side
    # plate at length 2.35 with the bore at y .17 (973.2 px per gun unit; the left plate agrees
    # within .01 on every landmark; procedural numbers in brackets). yMax .75 is the drawn scope top,
    # y .634 (.617), plus .10: the burst's hammer came out .076 above what its plate drew. There is
    # NO magazine on this gun, so the lowest point is the pistol grip's brass cap at y -.308 (z +.01),
    # with the butt toe -.249 (z +.40) and the trigger-guard bow -.157 (z -.22); yMin's ceiling is
    # -.18 -- the drawn depth plus the .105 the burst's whole lower half came out shallow -- not the
    # grip's own -.22. The grip rule is the long guns' stock-wrist band with zFrom -.02, which drops
    # the trigger guard (z -.36 .. -.09, all ahead of it); on the plate the band holds the grip flank
    # and the butt underside with centroid z .213 and z-extent .462, and it stays non-empty whether
    # the build is faithful or the burst's .10 shallow. The barrel band z -1.62..-1.36 is bare tube:
    # the brass muzzle ring ends at z -1.690 and the fore-end's wooden nose is drawn at z -1.216 (the
    # telescope objective is further back still, its front edge at z -1.071) but Meshy BUILT the
    # wood running forward to z -1.34, so the band clears the ring by .07 and the built wood by .02;
    # on the plates it measures centroid y .170 -- exactly the bore -- and on the built mesh .114,
    # the tube's cross-section centre there. Its action is the bolt handle: a steel rod running rearward out of
    # the lock's rear face to a brass ball knob at z +.028 .. +.122, y .148 .. .248 (centre z .080,
    # y .212, r .048), which the frame code TRANSLATES straight back rather than rotates
    # (position.z += .17 * open, built with --hinge-axis 0 0 1, an identity hinge frame), so unlike
    # the magazines the action y band is positive: [.05, .35] is the knob plus .10 either way, and
    # that ceiling also keeps the hammer (top y .432) out of a split that reached too far. reach .25
    # covers the knob's far side from a pivot at the bolt's root (~.18) and refuses one that
    # swallowed the lock plate (.37 long). Measure the built mesh before calling any of this a bad
    # reconstruction -- the burst's stock-wrist band moved on the mesh, not the plate.
    'longshot': {
        'bounds': {'zMin': (-2.00, -1.80), 'zMax': .60, 'yMin': (-.60, -.18), 'yMax': .75, 'absX': .30},
        'muzzle': {'zBelowMin': -.02, 'zAboveMin': .06, 'y': (.12, .22), 'absX': .03},
        'grip': {'band': (-.20, -.08), 'zFrom': -.02, 'absCentroidX': .06, 'centroidZ': (.05, .40),
                 'extentX': .30, 'extentZ': .50},
        'barrel': {'band': (-1.62, -1.36), 'centroidY': (.02, .25), 'absCentroidX': .03,
                   'extentX': .25, 'extentY': .40},
