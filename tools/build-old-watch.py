...
from environment_kit import (MATERIALS, PREFABS, KitMesh, clamp, game_to_blender,
                             unit, rotate_y, png, tile_noise,
                             create_textured_material, export_glb)
...
def create_materials(temp):
    ...
    for name, kind, roughness, tint in (
        ...
        create_textured_material(name, image_sets[kind], roughness, tint,
                                 double_sided=name == 'needle_foliage',
                                 normal_strength=.7 if kind == 'foliage' else 1)
