...
import { createReefFishSchools } from './underwater-fish.js';
import { createReefWayfinding } from './underwater-wayfinding.js';
import { createSunkenManor } from './sunken-manor.js';
...
function makeDiscoveryArt(resources, discovery) {
  ...
  switch (discovery.id) {
    ...
    case 'manor-chandelier': b.add('sphere', [0, 0, 0], [.28, .18, .28], [0, 0, 0], '#c7b689'); for (let i = 0; i < 6; i++) { const a = i * TAU / 6; b.line([0, .1, 0], [Math.cos(a) * .55, .45, Math.sin(a) * .55], .035, '#5e897e'); } break;
  }
...
function makeCacheCradle(resources, cache) {
  const group = new THREE.Group(); group.name = `sunken-reach-cache-${cache.id}-cradle`; group.position.set(cache.x, cache.y, cache.z); const b = resources.batch(cache.region === 'ember-vents' ? 'basalt' : 'stone'), floor = (cache.region === 'sunken-manor' ? 13.05 : floorY(cache.x, cache.z)) - cache.y;
...
export function createUnderwaterPresentation({ palette, heightAt }) {
  const resources = createReefResources(palette), habitat = createReefHabitat(resources), fish = createReefFishSchools(), wayfinding = createReefWayfinding(resources), manor = createSunkenManor(resources);
  const group = new THREE.Group(); group.name = 'sunken-reach-six-biome-expedition'; group.add(habitat.group, fish.group, wayfinding.group, manor.group, makeWreckLandmark(resources), makeLandmarkSolids(resources), makeLandmarkSupports(resources, REEF_LANDMARK_SUPPORTS), makeRegionalLandmarkCompositions(resources, REGION));
  ...
  const bubbles = [], bubbleMaterial = ownedMaterial('#d5fff1', { transparent: true, opacity: .48, depthWrite: false });
  const bubbleBatch = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 6, 5), bubbleMaterial, 58); bubbleBatch.name = 'sunken-reach-batched-bubbles'; bubbleBatch.frustumCulled = false; group.add(bubbleBatch);
  const originalBubbleRegions = REEF_REGIONS.filter(region => region.id !== 'sunken-manor');
  for (let i = 0; i < 58; i++) { const region = originalBubbleRegions[i % originalBubbleRegions.length], atVent = region.id === 'ember-vents', mainMouth = atVent && i % 3 === 1; bubbles.push({ index: i, size: .04 + i % 3 * .023, x: mainMouth ? 101 + Math.sin(i) * .9 : region.x - 20 + i * 7 % 40, z: mainMouth ? -55 + Math.cos(i) * .8 : region.z - 18 + i * 11 % 36, phase: i * .77, baseY: mainMouth ? 28.55 : atVent ? 4.5 + i % 4 * 1.9 : .25, rise: atVent ? 5 + i % 4 : 5 + i % 12 }); }
  const instance = new THREE.Object3D(); instance.scale.setScalar(0); instance.updateMatrix(); for (let i = 0; i < bubbles.length; i++) bubbleBatch.setMatrixAt(i, instance.matrix); bubbleBatch.instanceMatrix.needsUpdate = true;
  ...
  return { group, diveMarker,
    update(time, snapshot = {}, { lowQuality: low = false, reducedMotion = false, player = null } = {}) {
      ...
      habitat.update(time, { lowQuality, reducedMotion, player }); fish.update(time, { lowQuality, reducedMotion, player }); wayfinding.update(time, { lowQuality, reducedMotion, player }); current.rotation.y = reducedMotion ? 0 : time * .65; current.position.y = REEF_EXIT.y - 1.45 + (reducedMotion ? 0 : Math.sin(time * 1.4) * .06);
    ...
    getStats() { const fishStats = fish.getStats(); return { ready: true, kind: 'sunken-reach', wreckSolids: (REEF_WRECK_SOLIDS || REEF_SOLIDS.filter(solid => solid.id.startsWith('wreck-'))).length, landmarkSolids: REEF_LANDMARK_SOLIDS.length, landmarkSupports: REEF_LANDMARK_SUPPORTS.length, regions: REEF_REGIONS.length, discoveries: discoveries.length, caches: caches.length, encounters: REEF_ENCOUNTERS.length, events: events.length, bubbles: bubbles.length, fish: fishStats.fish, fishSchools: fishStats.schools, chest: REEF_CHEST.id, lowQuality, resources: { materials: Object.keys(resources.materials).length, texturesPerMaterial: 3 }, habitat: habitat.getStats(), wayfinding: wayfinding.getStats(), manor: manor.getStats() }; },
    dispose() { if (disposed) return; disposed = true; fish.dispose(); wayfinding.dispose(); manor.dispose(); for (const shaft of shafts.children) shaft.userData.dispose?.(); resources.dispose(); },
  };
}
