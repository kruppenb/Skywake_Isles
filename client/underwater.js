...
export function createUnderwaterPresentation({ palette, heightAt }) {
  const resources = createReefResources(palette), habitat = createReefHabitat(resources), fish = createReefFishSchools(), wayfinding = createReefWayfinding(resources);
  const group = new THREE.Group(); group.name = 'sunken-reach-six-biome-expedition'; group.add(habitat.group, fish.group, wayfinding.group, makeWreckLandmark(resources), makeLandmarkSolids(resources), makeLandmarkSupports(resources, REEF_LANDMARK_SUPPORTS), makeRegionalLandmarkCompositions(resources, REGION));
