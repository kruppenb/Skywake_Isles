...
const watchURL = '/assets/old-watch/kit.glb', farmURL = '/assets/windward-farm/kit.glb', marketURL = '/assets/tideglass-market/kit.glb', harborURL = '/assets/saltwind-harbor/kit.glb';
function fixture() {
...
test('lighting normalizes overlaps and retains the farm exclusion only when the farm fails', () => {
  ...
  assert.deepEqual(environmentWeights({ x: NaN, z: 0 }, { oldWatchReady: true, farmReady: true }), { baseline: 1, oldWatch: 0, windwardFarm: 0, tideglassMarket: 0, saltwindHarbor: 0 });
  let last;
  for (let i = 0; i <= 330; i++) {
...
test('coastal lighting depends on market readiness and restores the baseline aboard and beyond the bounded area', () => {
  ...
  const player = { x: -29, z: 39, mode: 'ground' }, baseline = { baseline: 1, oldWatch: 0, windwardFarm: 0, tideglassMarket: 0, saltwindHarbor: 0 };
...
test('registered island payload and texture costs match the committed kit manifests', async () => {
  assert.deepEqual(Object.keys(ENVIRONMENT_ASSET_REGISTRY), [watchURL, farmURL, marketURL, harborURL]);
